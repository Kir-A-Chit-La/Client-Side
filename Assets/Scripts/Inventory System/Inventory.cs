...
{
    public event Action<int, string, int> OnItemSlotChangeEvent;
    ...
    private void OnItemSlotChange(BaseItemSlot itemSlot)
    {
        for(int i = 0; i < itemSlots.Count; i++)
        {
            if(itemSlot.Item != null && itemSlots[i].Item != null && itemSlots[i].Item.Id == itemSlot.Item.Id)
                OnItemSlotChangeEvent?.Invoke(i, itemSlot.Item.Id, itemSlot.Amount);
        }
    }
    private void SetMaxSlots(int value)
    {
        ...
        if(_maxSlots < itemSlots.Count)
        {
            ...
            {
                UnsubscribeFromEvents(itemSlots[i]);
